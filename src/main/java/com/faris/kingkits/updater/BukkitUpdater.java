...
public class BukkitUpdater {
    ...
    // If the file name contains one of these, don't update.
    private static final String[] NO_UPDATE_NAME = {"Special"};
    ...
    private BukkitUpdater.UpdateResult result = BukkitUpdater.UpdateResult.SUCCESS;
     ...
     */
    public BukkitUpdater(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback) {
     ...
     * @param announce True if the program should announce the progress of new updates in console.
     */
    public BukkitUpdater(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback, boolean announce) {
        this.plugin = plugin;
        this.type = type;
        ...
        try {
            this.url = new URL(BukkitUpdater.HOST + BukkitUpdater.QUERY + this.id);
    ...
    private boolean versionCheck() {
        ...
        if (this.type != UpdateType.NO_VERSION_CHECK) {
            ...
            if (title.split(DELIMETER).length == 2) {
                ...
                if (this.hasTag(localVersion) || this.hasName(title) || !this.shouldUpdate(localVersion, remoteVersion)) {
    ...
    /**
     * Evaluate whether the plugin name is marked showing that it should not be updated by this program.
     *
     * @param name a version number to check for tags in.
     * @return true if updating should be disabled.
     */
    private boolean hasName(String name) {
        for (final String string : BukkitUpdater.NO_UPDATE_NAME) {
    ...
    public interface UpdateCallback {
         ...
         */
        void onFinish(BukkitUpdater updater);
    }
